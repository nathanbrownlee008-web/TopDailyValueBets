...
function openVipModal(){
  ...
  const saved = normalizeVipEmail((localStorage.getItem('vip_email') || "").trim());
  if(vipEmailEl) vipEmailEl.value = saved || vipEmailEl.value || "";
  if(vipPasswordEl) vipPasswordEl.value = "";
...
async function ensureVipPasswordAccount(email, password){
  ...
  if(cleanPassword.length < 6) throw new Error("Enter your VIP password (minimum 6 characters).");
  ...
  if(msg.includes("already") || msg.includes("exists") || msg.includes("registered") || msg.includes("user already")){
    const retry = await client.auth.signInWithPassword({ email: cleanEmail, password: cleanPassword });
...
async function restoreVipAccess(){
  ...
  if(password.length < 6){
    if(vipErrorEl) vipErrorEl.textContent = "Enter your VIP password to restore access.";
    return;
  }
  try{
    toggleVipPlanButtons(true);
    await ensureVipPasswordAccount(email, password);
    const active = await forceVipRefreshNow(email);
    if(active){
      if(vipErrorEl) vipErrorEl.textContent = "VIP restored.";
    }else{
      if(vipErrorEl) vipErrorEl.textContent = "No active VIP subscription found for that email yet.";
    }
  }catch(err){
    if(vipErrorEl) vipErrorEl.textContent = err?.message || "Could not restore VIP.";
  }finally{
    toggleVipPlanButtons(false);
  }
}
...
function toggleVipPlanButtons(disabled){
  if(vipMonthlyEl) vipMonthlyEl.disabled = !!disabled;
  if(vipYearlyEl) vipYearlyEl.disabled = !!disabled;
  if(vipRestoreEl) vipRestoreEl.disabled = !!disabled;
}

async function checkVIP(){
  const email = normalizeVipEmail((localStorage.getItem('vip_email') || "").trim());
  if(!email){
    vipActive = false;
    setVipUI(false,"");
    return false;
...
async function startCheckout(plan){
  ...
  if(password.length < 6){
    if(vipErrorEl) vipErrorEl.textContent = "Create or enter your VIP password first.";
    return;
  }
  try{
    toggleVipPlanButtons(true);
...
if(vipPasswordEl){
  vipPasswordEl.addEventListener('keydown',(e)=>{
    if(e.key === 'Enter') restoreVipAccess();
  });
}
