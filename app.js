  ...
  try{
    ...
    let html = `
      <div class="tdt-table-wrap">
        <table class="tdt-table tdt-table-fit tdt-table-responsive">
          <thead>
            <tr>
    ...
    rows.forEach(row=>{
      ...
      html += `
        <tr class="tdt-row ${result}">
          ...
          <td class="tdt-stake"><span class="tdt-num">£${Number(row.stake || 0).toFixed(2)}</span></td>
          <td class="tdt-odds"><span class="tdt-num">${row.odds != null && row.odds !== '' ? escapeHtml(String(row.odds)) : '-'}</span></td>
    ...
    set("tdtAvgOdds", settledRows.length?(totalOdds/settledRows.length).toFixed(2):0);
