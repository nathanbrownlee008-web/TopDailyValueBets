...
borderColor:"#22c55e",
borderWidth:1.8,
pointRadius: history.length ? 3 : 0,
pointHoverRadius: history.length ? 5 : 0,
...
const gameDayKey = normalizeDateOnly(gameDate) || normalizeDateOnly(row.created_at) || toLocalYMD(new Date());
dailyBankrollMap[gameDayKey] = bankroll;
html+=`<tr>
...
// Daily chart: one point per end of day
const dailyEntries = Object.entries(dailyBankrollMap).sort((a,b)=>a[0].localeCompare(b[0]));
const dailyLabels = dailyEntries.map(([dayKey]) => fmtDayLabel(dayKey));
const dailyHistory = dailyEntries.map(([, value]) => Number(Number(value).toFixed(2)));
