...
// ===== BANKROLL GROWTH + ROI SUBLABEL PATCH =====
(function(){
  function getStartingBankrollValue(){
    const input = document.getElementById('startingBankroll');
    const raw = input ? Number(input.value) : Number(localStorage.getItem('startingBankroll') || 100);
  ...
  function ensureSubText(el, className, text){
    if(!el) return null;
    let sub = el.querySelector(`.${className}`);
    if(!sub){
  ...
  function patchTrackerStatCards(){
    try{
      // MY TRACKER: add Growth % under bankroll figure
      const bankrollValueEl = bankrollElem ? bankrollElem.closest('strong') : null;
      if(bankrollValueEl){
        const start = getStartingBankrollValue();
        const bankrollNow = Number((bankrollElem.textContent || '0').replace(/[^\d.-]/g,'')) || 0;
        const growthPct = start > 0 ? ((bankrollNow - start) / start) * 100 : 0;
        ensureSubText(bankrollValueEl.parentElement, 'bankroll-growth-sub', `Growth ${formatSignedPct(growthPct)}`);
      }

      // MY TRACKER: ROI label helper
      const roiCard = roiElem ? roiElem.closest('.stat-card') : null;
      if(roiCard){
        ensureSubText(roiCard, 'roi-subnote', 'Based on £ staked');
      }

      // TDT TRACKER: ROI label helper
      document.querySelectorAll('.performance-card, .tdt-performance, .card').forEach(card=>{
        const txt = (card.textContent || '').toLowerCase();
        if(txt.includes('performance') && txt.includes('roi') && txt.includes('avg odds')){
          let roiRow = Array.from(card.querySelectorAll('*')).find(el => (el.textContent || '').trim() === 'ROI');
          if(roiRow){
            const holder = roiRow.parentElement || roiRow;
            ensureSubText(holder, 'roi-subnote', 'Based on £ staked');
          }
        }
      });
    }catch(e){
      console.error('bankroll/roi ui patch failed', e);
    }
  }

  // run now + after rerenders
  setTimeout(patchTrackerStatCards, 250);
  setTimeout(patchTrackerStatCards, 900);

  const uiPatchObserver = new MutationObserver(() => patchTrackerStatCards());
  uiPatchObserver.observe(document.body, { childList:true, subtree:true });

  document.addEventListener('input', (e)=>{
    if(e.target && e.target.id === 'startingBankroll'){
      patchTrackerStatCards();
    }
  });
})();
