...
/* ===== TDT PRO ANALYTICS UPGRADES ===== */

function computeTdtEnhancements(){
  try{
    if(!window.tdtRowsCache || !Array.isArray(window.tdtRowsCache)) return;

    const rows=window.tdtRowsCache
    const dayProfit={}
    const marketProfit={}

    rows.forEach(r=>{
      const date=(r.created_at||r.bet_date||"").slice(0,10)
      ...
      if(!dayProfit[date]) dayProfit[date]=0
      dayProfit[date]+=profit

      const m=r.market||"Unknown"
      if(!marketProfit[m]) marketProfit[m]=0
      marketProfit[m]+=profit
    })

    let bestMarket=null
    let bestProfit=-Infinity

    Object.entries(marketProfit).forEach(([m,p])=>{
      if(p>bestProfit){bestProfit=p;bestMarket=m}
    })

    if(bestMarket){
      let el=document.getElementById("bestMarketCard")
      if(!el){
        el=document.createElement("div")
        el.id="bestMarketCard"
        const stats=document.querySelector("#tdtTrackerSection .stats-grid")
        if(stats) stats.after(el)
      }
      el.innerHTML="🏆 Best Market: "+bestMarket+" ( £"+bestProfit.toFixed(2)+" )"
    }

    Object.entries(dayProfit).forEach(([day,p])=>{
      const nodes=document.querySelectorAll(".tracker-toggle,.tdt-day-header,.tdt-day")
      nodes.forEach(n=>{
        if(n.textContent.includes(day)){
          if(!n.querySelector(".tdt-profit-chip")){
            const chip=document.createElement("span")
            chip.className="tdt-profit-chip"
            chip.textContent=(p>=0?"+£":"-£")+Math.abs(p).toFixed(2)
            n.appendChild(chip)
          }
          n.classList.toggle("profit",p>0)
          n.classList.toggle("loss",p<0)
        }
      })
    })

  }catch(e){console.error("TDT enhancements error",e)}
}

const tdtObserver=new MutationObserver(()=>{
  setTimeout(computeTdtEnhancements,200)
})

document.addEventListener("DOMContentLoaded",()=>{
  const target=document.getElementById("tdtTrackerTable")
  if(target) tdtObserver.observe(target,{childList:true,subtree:true})
})
