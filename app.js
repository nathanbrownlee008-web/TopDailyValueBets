...
// ===== ADD BOOKIE TO TRACKER CARDS (SAFE PATCH) =====
(function(){

  function addBookieToTracker(){
    try{
      const cards = document.querySelectorAll('.tracker-card, .bet-card');

      cards.forEach(card=>{
        // don't duplicate
        if(card.querySelector('.tracker-bookie')) return;

        // try find bookie from dataset or text
        let bookie = '';

        // check stored attribute (best case)
        if(card.dataset && card.dataset.bookie){
          bookie = card.dataset.bookie;
        }

        // fallback: try grab from internal text (odds row etc)
        if(!bookie){
          const text = card.textContent || '';
          if(text.includes('Ladbrokes')) bookie = 'Ladbrokes';
          if(text.includes('Goldenbet')) bookie = 'Goldenbet';
        }

        if(!bookie) return;

        // find league line
        const leagueEl = card.querySelector('.tracker-kickoff, .bet-league');

        if(!leagueEl) return;

        // create pill
        const pill = document.createElement('div');
        pill.className = 'tracker-bookie';
        pill.textContent = bookie;

        // insert AFTER league
        leagueEl.insertAdjacentElement('afterend', pill);

      });

    }catch(e){
      console.log('bookie patch error', e);
    }
  }

  // run after render
  window.addEventListener('load', ()=>{
    setTimeout(addBookieToTracker,
