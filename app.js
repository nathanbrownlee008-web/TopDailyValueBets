    ...
    if(betsTbody){
      ...
      <tr class="${locked ? 'bet-row--locked' : ''}">
        <td class="table-match-cell">${
          locked
            ? `<b>${escapeHtml(row.match||'')}</b>`
            : `<div class="table-match-wrap">${
                leagueName
                  ? `<div class="table-match-league">🏴 ${escapeHtml(leagueName)}</div>`
                  : ''
              }<div class="table-match-name">${escapeHtml(row.match||'')}</div></div>`
        }</td>
