...
function _rowGameDateISO(row){
  const raw = row.match_date_date || row.bet_date || row.match_date || row.created_at;
  if(!raw) return "";
...
function _buildTrackerTableHTML(rows){
  ...
  (rows || []).forEach(row=>{
    ...
    const dateLabel = fmtLabel(row.match_date_date || row.bet_date || row.match_date || row.created_at);
...
(function(){
  ...
  function trackerRawDate(row){
    return row.match_date_date || row.bet_date || row.match_date || row.created_at;
  }
