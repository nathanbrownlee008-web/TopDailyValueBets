...
// ===== FINAL VIP NO FLICKER FIX =====
let vipLoadedOnce = false;

async function runVipPreviewOnce() {
  if (vipLoadedOnce) return;
  vipLoadedOnce = true;

  const el = document.getElementById('vipPromoStats');
  if (!el) return;

  try {
    const res = await fetch(`${SUPABASE_URL}/rest/v1/tdt_tracker?select=*&order=created_at.asc`, {
      headers: {
        apikey: SUPABASE_KEY,
        Authorization: `Bearer ${SUPABASE_KEY}`
      }
    ...
    const settled = data.filter(x => x.result !== 'pending');
    ...
    settled.forEach(b => {
      ...
      let pr = 0;
      if (b.result === 'won') { pr = stake * (odds - 1); w++; }
      if (b.result === 'lost') { pr = -stake; l++; }

      s += stake;
      p += pr;
    });

    const roi = s ? (p / s * 100) : 0;

    el.textContent =
      `${settled.length} bets • ${w}-${l} • £${p.toFixed(2)} • ${roi.toFixed(1)}% ROI`;

  } catch (e) {
    el.textContent = 'Error loading proof';
  }
}

setTimeout(runVipPreviewOnce, 500);
// ===== END FIX =====
