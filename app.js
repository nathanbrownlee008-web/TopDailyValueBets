...
function getMarketIcon(market){
  ...
  if(m.includes("shot on target") || m.includes("shots on target") || m.includes("sot")) return "🎯";
  if(m.includes("foul")) return "⚠️";
  if(m.includes("offside")) return "🚫";
  if(m.includes("asian handicap") || m.includes("handicap")) return "⚖️";
  if(m.includes("match winner") || m.includes("to win") || m.includes("1x2") || m == "home" || m == "away" || m == "draw") return "🏆";
  if(m.includes("goal") || m.includes("fhg") || m.includes("fgh")) return "⚽";
  return "•";
}
...
function getMarketIconSvg(market){
  ...
  const wrap = (inner, vb='0 0 24 24') =>
    `<span class="vb-svg-icon" aria-hidden="true"><svg viewBox="${vb}" fill="none" xmlns="http://www.w3.org/2000/svg">${inner}</svg></span>`;

  if(m.includes("throw")){
    return wrap('<path d="M7 8L3 12L7 16" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/><path d="M17 8L21 12L17 16" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/><path d="M4 12H20" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"/>');
  }
  if(m.includes("corner")){
    return wrap('<path d="M6 20V5" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"/><path d="M8 6C11 5 13 8 16 7C17.3 6.6 18.2 5.8 19 5V12C18.2 12.8 17.3 13.6 16 14C13 15 11 12 8 13V6Z" fill="currentColor" opacity="0.95"/>');
  }
  if(m.includes("card") || m.includes("booking")){
    return wrap('<rect x="6" y="4" width="12" height="16" rx="2.5" fill="currentColor"/>');
  }
  if(m.includes("btts") || m.includes("both teams to score")){
    return wrap('<path d="M12 3L18.5 7V17L12 21L5.5 17V7L12 3Z" stroke="currentColor" stroke-width="2"/><path d="M9 12.5L11 14.5L15.5 10" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/>');
  }
  if(m.includes("shot on target") || m.includes("shots on target") || m.includes("sot")){
    return wrap('<circle cx="12" cy="12" r="7.5" stroke="currentColor" stroke-width="2"/><circle cx="12" cy="12" r="3" fill="currentColor"/><path d="M12 2.5V5" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><path d="M12 19V21.5" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><path d="M2.5 12H5" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><path d="M19 12H21.5" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>');
  }
  if(m.includes("foul")){
    return wrap('<path d="M12 3L21 19H3L12 3Z" stroke="currentColor" stroke-width="2"/><path d="M12 9V13" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"/><circle cx="12" cy="16.3" r="1.2" fill="currentColor"/>');
  }
  if(m.includes("offside")){
    return wrap('<path d="M4 6V18" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"/><path d="M6 7H18L14 11H6V7Z" fill="currentColor"/><path d="M8 17L17 8" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"/>');
  }
  if(m.includes("asian handicap") || m.includes("handicap")){
    return wrap('<path d="M12 4L5 8V12C5 16.5 8 19.6 12 20.5C16 19.6 19 16.5 19 12V8L12 4Z" stroke="currentColor" stroke-width="2"/><path d="M8 13H16" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"/><path d="M12 9V17" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"/>');
  }
  ...
  if(m.includes("goal") || m.includes("fhg") || m.includes("fgh")){
    return wrap('<circle cx="12" cy="12" r="7.5" stroke="currentColor" stroke-width="2"/><path d="M12 7.5L15 9.3L14.4 12.8L9.6 12.8L9 9.3L12 7.5Z" fill="currentColor"/><path d="M9.6 12.8L8.2 15.8M14.4 12.8L15.8 15.8M9 9.3L6.2 8.7M15 9.3L17.8 8.7" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/>');
  }
  return wrap('<circle cx="12" cy="12" r="2.2" fill="currentColor"/><circle cx="12" cy="12" r="7.5" stroke="currentColor" stroke-width="2" opacity="0.55"/>');
}
  ...
  <div class="card bet-card ${row.high_value ? 'bet-card--hv' : ''} ${locked ? 'bet-card--locked' : ''}">
    <div class="bet-teaser">
      ...
      <div class="bet-meta bet-meta--market-row">
        ${locked ? `<span class="bet-market bet-market--locked">🔒 Hidden market</span>` : `<span class="bet-market">${getMarketIconSvg(row.market)}<span class="bet-market-text">${escapeHtml(row.market || '')}</span></span>`}
      </div>
      ...
      <tr class="${locked ? 'bet-row--locked' : ''}">
        ...
        <td>${
          locked
            ...
            : `<div class="table-market-wrap"><div class="table-market-line table-market-pill"><span class="table-market-icon">${getMarketIconSvg(row.market||'')}</span><span class="table-market-text">${escapeHtml(row.market||'')}</span></div></div>`
        }</td>
