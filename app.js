...
function isValueBetActiveToday(row){
  ...
  // no usable dates -> still show the bet
    ...
    months.forEach((monthEntry, monthIndex)=>{
      ...
      weekEntries.forEach(([weekLabel, weekEntry], weekIndex)=>{
        ...
        dayEntries.forEach(([dayLabel, dayRows], dayIndex)=>{
          ...
          dayRows.forEach(row=>{
            html += `
              <div class="tracker-grid-card result-${trackerEsc(row.result || 'pending')}">
                ...
                <div class="tracker-grid-meta tracker-grid-meta--single-row">
                  <div class="tracker-grid-market-slot">
                    ...
                    <div class="tracker-grid-market-inline">
                      ${trackerEsc(getMarketIcon(row.market, row.sport || getBetSport(row)) ? `${getMarketIcon(row.market, row.sport || getBetSport(row))} ${row.market || "—"}` : (row.market || "—"))}
                    </div>
                  </div>
          ...
          dayRows.forEach(row=>{
            ...
            html += `
                      <tr>
                        ...
                        <td class="tracker-desktop-market">${trackerEsc(getMarketIcon(row.market, row.sport || getBetSport(row)) ? `${getMarketIcon(row.market, row.sport || getBetSport(row))} ${row.market || "—"}` : (row.market || "—"))}</td>
                        <td>
                          <input 
                            type="number" 
                            value="${Number(row.stake || 0)}" 
