...
function trackerTagOptions(selectedTag){
  const tags = ["", "goals", "cards", "corners", "player", "acca", "live", "other"];
  return tags.map(tag=>{
    const label = tag ? (tag.charAt(0).toUpperCase() + tag.slice(1)) : "—";
    return `<option value="${tag}" ${selectedTag===tag?"selected":""}>${label}</option>`;
  }).join("");
}
...
<td>
<select class="tracker-tag-select" onchange="updateTag('${row.id}',this.value)">
${trackerTagOptions(row.tags || "")}
</select>
</td>
<td>
<input class="tracker-notes-input" type="text" value="${escapeHtml(row.notes || "")}" placeholder="Notes" onchange="updateNotes('${row.id}',this.value)">
</td>
<td>
<select 
...
let html="<table><tr><th class='date-col'>Date</th><th>Match</th><th>Stake</th><th>Tag</th><th>Notes</th><th>Result</th><th class='profit-col'>Profit</th></tr>";
