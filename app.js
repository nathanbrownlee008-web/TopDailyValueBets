  ...
  try{
    ...
    monthGroups.forEach((group, idx)=>{
      ...
      group.rows.forEach(row=>{
        const result = String(row.result || "pending").toLowerCase();
        const resultIcon = result === "won" ? "✅" : result === "lost" ? "❌" : "⏳";
        const rowDayKey = (typeof getTdtRowDayKey === "function") ? getTdtRowDayKey(row) : "";
        const rowDateLabel = (typeof fmtTdtDayHeader === "function") ? fmtTdtDayHeader(rowDayKey) : escapeHtml(String(row.bet_date || row.created_at || ""));

        if(rowDateLabel !== lastDateLabel){
          html += `
          <tr class="tdt-date-row">
            <td colspan="5">${escapeHtml(rowDateLabel)}</td>
          </tr>
          `;
          lastDateLabel = rowDateLabel;
        }
