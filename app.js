...
<div class="bet-lock-wrap">
  <div class="card bet-card ${row.high_value ? 'bet-card--hv' : ''}${liveGlowClass} ${locked ? 'bet-card--locked' : ''}">
  ...
  ${locked ? '<button class="vip-overlay" type="button" data-open-vip="1">🔒 Unlock VIP</button>' : ''}
    ...
    if(betsTbody){
      ...
      <tr class="${locked ? 'bet-row--locked' : ''}">
        ...
        <td><span class="pill${valueClass}">${escapeHtml((valueBadge ? valueBadge + " " : "") + valTxt)}</span></td>
...
function notifyForNewVisibleBets(rows){
  ...
  const vRaw = latest?.value_pct ?? latest?.value_percent ?? latest?.value_percentage ?? latest?.value;
  const vNum = vRaw != null ? Number(vRaw) : null;
  const badge = vNum != null && !Number.isNaN(vNum)
    ? (vNum >= 12 ? '🚀 ' : (vNum >= 8 ? '💎 ' : (vNum >= 6 ? '🔥 ' : '')))
    : '';
  const valueLine = vNum != null && !Number.isNaN(vNum) ? ` • ${badge}${vNum.toFixed(1)}%` : '';
  sendBetNotification('New Top Daily Tips bet', `${latest.match || 'New bet'} • ${latest.odds || ''}${valueLine}`);
}
