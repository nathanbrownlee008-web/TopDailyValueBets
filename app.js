...
/* === personal tracker grouped renderer override === */
function toggleTrackerMonthGroup(btn){
  const body = btn && btn.nextElementSibling;
  if(!body) return;
  const arrow = btn.querySelector(".myt-month-arrow");
  const open = body.style.display !== "none";
  body.style.display = open ? "none" : "block";
  if(arrow) arrow.textContent = open ? "▶" : "▼";
}

function toggleTrackerDayGroup(btn){
  const body = btn && btn.nextElementSibling;
  if(!body) return;
  const arrow = btn.querySelector(".myt-day-arrow");
  const open = body.style.display !== "none";
  body.style.display = open ? "none" : "block";
  if(arrow) arrow.textContent = open ? "▶" : "▼";
}
...
loadTracker = async function(){
  ...
  rows.forEach(row=>{
    ...
    if(!monthMap.has(monthKey)){
      monthMap.set(monthKey, { label: monthLabel, days:new Map(), wins:0, losses:0, resolved:0, rows:0 });
    }
    const month = monthMap.get(monthKey);
    month.rows += 1;
    if(row.result==="won"){ month.wins += 1; month.resolved += 1; }
    if(row.result==="lost"){ month.losses += 1; month.resolved += 1; }

    if(!month.days.has(dayKey)) month.days.set(dayKey, []);
    month.days.get(dayKey).push(row);
  });

  let html = '<div class="myt-wrap">';
  const monthEntries = Array.from(monthMap.entries()).sort((a,b)=> a[0] < b[0] ? 1 : -1);

  monthEntries.forEach(([monthKey, month], monthIndex)=>{
    const wr = month.resolved ? Math.round((month.wins / month.resolved) * 100) : 0;
    html += `
      <div class="myt-month-card">
        <button class="myt-month-head" type="button" onclick="toggleTrackerMonthGroup(this)">
          <div class="myt-month-left">
            <div class="myt-month-title">${month.label}</div>
            <div class="myt-month-sub">${month.rows} results</div>
          </div>
          <div class="myt-month-right">
            <span class="myt-chip myt-chip-win">Won ${month.wins}</span>
            <span class="myt-chip myt-chip-loss">Lost ${month.losses}</span>
            <span class="myt-chip myt-chip-rate">Winrate ${wr}%</span>
            <span class="myt-month-arrow">${monthIndex===0 ? "▼" : "▶"}</span>
          </div>
        </button>
        <div class="myt-month-body" style="display:${monthIndex===0 ? "block" : "none"};">
    `;

    const dayEntries = Array.from(month.days.entries()).sort((a,b)=> new Date(b[1][0].match_date_date || b[1][0].bet_date || b[1][0].created_at) - new Date(a[1][0].match_date_date || a[1][0].bet_date || a[1][0].created_at));
    dayEntries.forEach(([dayLabel, dayRows], dayIndex)=>{
      html += `
        <div class="myt-day-group">
          <button class="myt-day-head" type="button" onclick="toggleTrackerDayGroup(this)">
            <span>${dayLabel}</span>
            <span class="myt-day-arrow">${dayIndex===0 ? "▼" : "▶"}</span>
          </button>
          <div class="myt-day-body" style="display:${dayIndex===0 ? "block" : "none"};">
            <table class="myt-table">
              <thead>
                <tr>
                  <th class="myt-col-match">Match</th>
                  <th class="myt-col-market">Market</th>
                  <th class="myt-col-stake">Stake</th>
                  <th class="myt-col-odds">Odds</th>
                  <th class="myt-col-result">Result</th>
                  <th class="myt-col-profit">Profit</th>
                </tr>
              </thead>
              <tbody>
      `;
      dayRows.forEach(row=>{
        ...
        html += `
          <tr>
            <td class="myt-match-cell">${row.match || ""}</td>
            <td class="myt-market-cell">${row.market || ""}</td>
            <td><input class="myt-input" type="number" value="${Number(row.stake||0)}" onchange="updateStake('${row.id}',this.value)"></td>
            <td><input class="myt-input" type="number" step="0.01" value="${Number(row.odds||0)}" onchange="updateOdds('${row.id}',this.value)"></td>
            <td>
  ...
  const monthProfitMap = {}, monthStakeMap2 = {};
  rows.forEach(r=>{
    const d = new Date(r.created_at || r.bet_date || Date.now());
    const key = d.getFullYear()+"-"+String(d.getMonth()+1).padStart(2,"0");
    monthProfitMap[key] = (monthProfitMap[key]||0) + rowProfit(r);
    monthStakeMap2[key] = (monthStakeMap2[key]||0) + Number(r.stake || 0);
  });
  const monthKeys = Object.keys(monthProfitMap).sort();
  ...
  const monthlyProfit = monthKeys.map(k=> monthProfitMap[k]);
  const monthlyROI = monthKeys.map(k=>{ const stake = monthStakeMap2[k] || 0; return stake ? (monthProfitMap[k] / stake) * 100 : 0; });
  renderMonthlyChart(monthlyProfit, monthlyROI, monthLabels);
