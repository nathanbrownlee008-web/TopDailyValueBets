...
/* ===== VALUE TABLE ENHANCER ===== */
(function () {
  function getRank(value) {
    if (value >= 6) return { label: "High", cls: "value-high" };
    if (value >= 3) return { label: "Med", cls: "value-med" };
    return { label: "Low", cls: "value-low" };
  }
  ...
  function enhanceValueTables() {
    ...
    tables.forEach((table) => {
      ...
      rows.forEach((row) => {
        ...
        if (!cells.length || !cells[valueIndex]) return;

        const valueCell = cells[valueIndex];
        const rawText = valueCell.textContent || "";
        const valueNum = parsePercent(rawText);
        if (valueNum === null) return;

        const rank = getRank(valueNum);

        valueCell.classList.remove("value-high", "value-med", "value-low");
        valueCell.classList.add("value-cell", rank.cls);

        let chip = valueCell.querySelector(".value-chip-auto");
        if (!chip) {
          chip = document.createElement("span");
          chip.className = "value-chip-auto";
          valueCell.innerHTML = "";
          valueCell.appendChild(chip);
        }

        chip.textContent = `${valueNum}%`;

        let badge = valueCell.querySelector(".value-rank-auto");
        if (!badge) {
          badge = document.createElement("span");
          badge.className = "value-rank-auto";
          valueCell.appendChild(badge);
        }

        badge.textContent = rank.label;
        badge.classList.remove("value-high", "value-med", "value-low");
        badge.classList.add(rank.cls);
      });
    });
  }
