...
async function forceVipRefreshNow(emailFromInput){
  ...
  if(!email || !email.includes("@")){
    if(vipErrorEl) vipErrorEl.textContent = "Enter your VIP email first.";
    return false;
  }
  ...
  try{
    // Kill any old auth session first
    ...
    const signedInEmail = normalizeVipEmail(signIn.data.user.email || "");
...
async function checkVIP(){
  ...
  try{
    ...
    const authedEmail = normalizeVipEmail(userData?.user?.email || "");

    // No signed-in user for this email = do not unlock VIP
    if(!authedEmail || authedEmail !== normalizeVipEmail(email)){
      vipActive = false;
...
let __vipPromoLoaded = false;
let __vipPromoLoading = false;

async function loadVipPromoProof(force=false){
  if(__vipPromoLoading) return;
  if(__vipPromoLoaded && !force) return;

  const statsEl = document.getElementById('vipPromoStats');
  if(!statsEl) return;

  __vipPromoLoading = true;
